#!/usr/bin/env python3
"""
Migration script to add 'authors' column to documents table in all databases.
"""
...
def migrate_database(db_path):
    """Add authors column to a database if it doesn't exist."""
    try:
        ...
        if 'authors' not in columns:
            print(f"Adding 'authors' column to {db_path}...")
            cursor.execute("ALTER TABLE documents ADD COLUMN authors TEXT")
            conn.commit()
            print(f"  ✓ Successfully added 'authors' column")
        else:
            print(f"  ✓ 'authors' column already exists in {db_path}")
...
def main():
    """Migrate all databases to add authors column."""
    print("Starting migration to add 'authors' column to all databases...")
    print()
