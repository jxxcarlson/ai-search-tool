...
def export_data(output_dir=None, compress=True):
    ...
    export_items = {
        "database": {
            "source": "storage/documents.db",
            "destination": "storage/documents.db",
            "type": "file"
        },
        "chroma_db": {
            "source": "storage/chroma_db",
            "destination": "storage/chroma_db",
            "type": "directory"
        },
        "documents": {
        ...
        "pdf_thumbnails": {
            "source": "storage/pdf_thumbnails",
            "destination": "storage/pdf_thumbnails",
            "type": "directory"
        },
        "document_ids": {
            "source": "storage/document_ids.json",
            "destination": "storage/document_ids.json",
            "type": "file"
        },
        "sample_documents": {
    ...
    # Gather statistics
    db_stats = get_database_stats(export_path / "storage/documents.db")
    manifest["statistics"] = {
        "document_count": db_stats["document_count"],
        "database_size": db_stats["database_size"],
        "pdf_count": count_files_in_directory(export_path / "storage/pdfs"),
        "thumbnail_count": count_files_in_directory(export_path / "storage/pdf_thumbnails"),
